...
async def run_command(discord, message, args, client, opt):
    ...
    a = ["at", "in", "with"]
    s = ["Sebi TV's", "Freddy's", "Rachael's", "CoolingTool's", "Baldi's", "Chuck E. Cheese's", "Scaries", "Scraps", "Mr. Hippo's", "Impostor", "Swacky's"]
    ...
    async def complete_name_gen(name):
        ...
        fnafnames = db.db_get("database/fnaf.json")

        fnafnames.setdefault(name, 0)
        
        fnafnames[name] = fnafnames[name] + 1
        db.db_write(fnafnames, "database/fnaf.json")
