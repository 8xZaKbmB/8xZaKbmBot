...
async def run_command(discord, message, args, client, opt):
    ...
    embed=discord.Embed(description="**prefixes: `"+" ".join(helper.prefixes[client.user.name])+"`**\nblue commands may only be available in guilds, or work differently outside guilds", color=client.theme_color)
    embed.set_author(name=f"{client.user.name} help",icon_url=f"{client.user.display_avatar.url}")
    embed.add_field(name=f"general {'<:udelinev3:1104797626882347184>'}", value="```ansi\n"+"```".join(general)+"```")
    embed.add_field(name=f"utility {'<:udelinev3:1104797626882347184>'}", value="```ansi\n"+", ".join(utility)+"```")
    embed.add_field(name=f"fun {'<:udelinev3:1104797626882347184>'}", value="```ansi\n"+", ".join(fun)+"```")
    embed.add_field(name=f"other {'<:udelinev3:1104797626882347184>'}", value="```ansi\n"+", ".join(other)+"```")
    embed.add_field(name=f"image {'<:udelinev3:1104797626882347184>'}", value="```ansi\n"+", ".join(image)+"```")
