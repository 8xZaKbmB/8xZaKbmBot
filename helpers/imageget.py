        ...
        if messageref.embeds:
            ...
            if "https://tenor.com/view/" in str(messageref.embeds[0].url):
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(f"https://tenor.googleapis.com/v2/posts?ids={tenor_id}&media_filter=minimal&limit=1&key={tenorapikey}") as resp:
                        outurl = json.loads(await resp.read())["results"][0]["media_formats"]["gif"]["url"]
                        async with aiohttp.ClientSession() as session:
